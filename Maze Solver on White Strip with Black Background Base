...
qtrrc.readLine(sensorValues);
    ...
    if ( found_left == 1)
    {
      ...
      delay(100);                        //TIME  BEFORE TAKING LEFT TURN DURING SEARCH RUN
    }
...
delay(200);
    ...
    for (int i = 0; i < path_length; i++)
    {
      ...
      if (path[i] == 'S')
      { 
        delay(200);                           //TIME  THE BOT TRAVELS STRAIGHT ON AN INTERSECTION DURING ACTUAL RUN
      }
      else if (path[i] == 'R')
      {
        delay(180);                          //TIME  BEFORE TAKING RIGHT TURN DURING ACTUAL  RUN
      }
      else {
       
        delay(180);                       //TIME  BEFORE TAKING LEFT TURN DURING ACTUAL RUN
      }
